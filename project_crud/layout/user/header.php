...
if (isset($_SESSION["email"]) && !empty($_SESSION["email"])) {
  ...
  $global_fetch_sql = "SELECT * FROM `users`  WHERE `user_id` ='{$user_id_session}'";

  $global_qwer = $conn->query($global_fetch_sql);

  if ($global_qwer->num_rows > 0) {

    $globalData = $global_qwer->fetch_assoc();
  }
}
?>
